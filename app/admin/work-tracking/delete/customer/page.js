...
export default function Customer() {
  ...
  const handleSubmit = async (event) => {
    ...
    const querySnapshot2 = await getDocs(collection(db, "customers"));
    let x = [];
    ...
    const searchedDoc = x.find((item) =>
      item.arr.find((item) => item.name === formData.name)
    );
    const updatedArr = searchedDoc.arr.filter(
      (item) => item.name !== formData.name
    );
    const docRef = doc(db, "customers", searchedDoc.id);
    await setDoc(docRef, {
      arr: updatedArr,
    });
    setFormData({
    ...
    toast.success("Müşteri başarıyla silindi.", { position: "top-center" });
  };
  const {
    customers,
    fiberTypes,
  ...
  return (
    ...
    >
      <div className="absolute inset-0 p-6 flex flex-col gap-2">
        ...
        >
          {loading ? <HashLoader size={20} color="#008000" /> : "Müşteriyi Sil"}
        </button>
      </div>
    </form>
  );
